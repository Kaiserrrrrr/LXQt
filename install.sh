...
# 3. Cleanup Script
(sudo pacman -Rns $(pacman -Qtdq) --noconfirm >/dev/null 2>&1 || true) && (lscpu | grep -iq "Intel" || sudo rm -rf /usr/lib/firmware/intel* 2>/dev/null) && (lspci | grep -iqE "amd|radeon" || sudo rm -rf /usr/lib/firmware/amdgpu /usr/lib/firmware/radeon 2>/dev/null) && (lspci | grep -iq "nvidia" || sudo rm -rf /usr/lib/firmware/nvidia 2>/dev/null) && (sudo rm -rf /usr/lib/firmware/{liquidio,qcom,netronome,nfp,mellanox,kaweth,ti-connectivity,mediatek,marvell,cavium,qed} 2>/dev/null) && (sudo find /usr/lib/modules -name "*.ko" -print0 | xargs -0 -P $(nproc) strip --strip-debug 2>/dev/null) && (sudo journalctl --vacuum-size=10M >/dev/null && sudo sed -i 's/#SystemMaxUse=/SystemMaxUse=10M/' /etc/systemd/journald.conf && sudo systemctl restart systemd-journald) && (rm -rf ~/.cache/* 2>/dev/null && sudo pacman -Scc --noconfirm >/dev/null 2>&1) && echo -e "\e[32m[✓] Cleanup Completed... Rebooting\e[0m"
