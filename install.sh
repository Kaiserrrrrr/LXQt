...
# 3. Cleanup Script
pacman -Qtdq >/dev/null && sudo pacman -Rns $(pacman -Qtdq) --noconfirm >/dev/null 2>&1 || true && STAY_INTEL=$(lscpu | grep -ic "Intel") && STAY_AMD=$(lspci | grep -icE "amd|radeon") && STAY_NVIDIA=$(lspci | grep -ic "nvidia") && [ "$STAY_INTEL" -eq 0 ] && sudo rm -rf /usr/lib/firmware/intel* 2>/dev/null || true && [ "$STAY_AMD" -eq 0 ] && sudo rm -rf /usr/lib/firmware/amdgpu /usr/lib/firmware/radeon 2>/dev/null || true && [ "$STAY_NVIDIA" -eq 0 ] && sudo rm -rf /usr/lib/firmware/nvidia 2>/dev/null || true && sudo rm -rf /usr/lib/firmware/{liquidio,qcom,netronome,nfp,mellanox,kaweth,ti-connectivity,mediatek,marvell,cavium,qed} 2>/dev/null && sudo find /usr/lib/modules -name "*.ko" -print0 | xargs -0 -P $(nproc) strip --strip-debug 2>/dev/null && sudo journalctl --vacuum-size=10M >/dev/null && sudo sed -i 's/#SystemMaxUse=/SystemMaxUse=10M/' /etc/systemd/journald.conf && sudo systemctl restart systemd-journald && rm -rf ~/.cache/* 2>/dev/null && sudo pacman -Scc --noconfirm >/dev/null 2>&1
